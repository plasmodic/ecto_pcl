...
Each of the above data types is implemented with the help of boost variants. To ease the difficulty in using them, we have created a number of stubs for typical ecto_pcl cells: 
 ...
 - PclCell (pcl_cell.hpp) - for cells that take a single PointCloud input.
 - PclCellDualInputs (pcl_cell_dual_inputs.hpp) - for cells with two PointCloud inputs.
 - PclCellWithNormals (pcl_cell_with_normals.hpp) - for cells that need a PointCloud and Normals (a FeatureCloud). 
...
The key aspects here are: 
 ...
 - To set the value of our output spore we have to use: xyz_cloud_variant_t(cloud.makeShared());
